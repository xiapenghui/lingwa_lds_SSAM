...
const Component = ({
  ...
  const TableModelsData = amoutInfo
  const { FromParams, 
    TableData,
     pagination, 
     tableLoading,
  ...
  const handleSearch = (e) => {
    form
      ...
      .then((values) => {
        const Params = {
          amountno: values.amountno, //单据号
          name:values.name,  //服务项目
          customer:values.customer,  //客户
          contractI: values.contractI == "" ? null : values.contractI,
  ...
  const PaginationComponentsChanger = (PageIndex, PageSize) => {
    console.log('---------------', form.getFieldValue('sdate1'))
    const Params = {
      amountno: form.getFieldValue("amountno"),
      name:form.getFieldValue("name"),
      customer:form.getFieldValue("customer"),
      contractI: form.getFieldValue("contractI") == "" ? null : form.getFieldValue("contractI"),
               ...
               <Select
                      ...
                      >
                        {selectDropDownData.contractiList != null ? selectDropDownData.contractiList.map(function (item, index) {
                          return <Select.Option key={index} value={item.label}>{item.value}</Select.Option>
                        }) : ""}
                      </Select>
              </Form.Item>
              </Col>
...
export default connect(({ amoutInfo }) => ({ amoutInfo }))(Component);
